...
impl Throttle {
    pub fn new(center: Point, height: u32, width: u32) -> Self {
        Self { center, height, width, percent: 0.5 }
    }
}
...
impl Paintable for Throttle {
    fn paint(&self, buffer: &mut Buffer) {
        let rectangle = Rectangle::new(self.center, Normalized::new(-0.1, 0.0), self.width, self.height);
        ...
        Line::new(left_point_half.unwrap(), right_point_half.unwrap()).paint(buffer);
        Line::new(left_point_state.unwrap(), right_point_state.unwrap()).paint(buffer);
    }
}